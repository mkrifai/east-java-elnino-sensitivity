...
*Figure 1: (a) Absolute seasonal precipitation deficit (mm) and relative anomaly (% of neutral baseline) across 8 historical El Niño episodes and cohort composites, partitioned into Pure El Niño ($n=5$) and Compound El Niño + IOD+ ($n=3$). (b) Standardized meteorological anomaly (Z-score) plotted against the neutral baseline climatological normal variability envelope ($|Z| \le 1.0\sigma$, shaded grey-blue) and the meteorological dryness threshold ($Z = -1.0$). Error bars indicate $\pm 1$ standard error of the mean (SEM). Cohort difference is non-significant ($p = 0.77$, Welch's t-test). All negative values use true typographical minus signs.*
...
* **Statistical Hypothesis Testing (Welch's Two-Sample t-Test)**:
  * **JJA Onset**: Pure mean anomaly ($-79.0\text{ mm}$) vs Compound mean anomaly ($-73.6\text{ mm}$), difference $\Delta = +5.4\text{ mm}$; Welch's $t = -0.309$, $p = 0.769$ (degrees of freedom $df \approx 5.6$). **Statistically non-significant**. Compound events are not drier during early dry season; pure events exhibited marginally greater JJA desiccation.
  * **SON Peak**: Pure mean anomaly ($-142.7\text{ mm}$) vs Compound mean anomaly ($-164.2\text{ mm}$), difference $\Delta = -21.5\text{ mm}$; Welch's $t = 0.331$, $p = 0.767$ ($df \approx 2.4$, standard error of difference $SE_\Delta = 64.8\text{ mm}$). **Statistically non-significant**.
* **Interpretation of Tail Risk & Physical Nuance**:
  While the cohort means are statistically indistinguishable due to the small sample size ($n=3$ compound vs $n=5$ pure) and large within-group variance (e.g., 2018 compound event was mild with SON anomaly of $-48.8\text{ mm}$ due to late IOD onset and early November convective recovery), **the Indian Ocean Dipole acts as an asymmetric tail-risk multiplier**:
